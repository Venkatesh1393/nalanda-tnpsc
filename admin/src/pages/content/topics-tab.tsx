...
export function TopicsTab() {
  ...
  const [editing, setEditing] = useState<AdminTopic | 'new' | null>(null)
  ...
  const { data, isPending, isError, refetch } = useQuery({
    queryKey: ['admin', 'topics', { search, subjectId, status, page }],
    queryFn: () =>
      listTopics({
  ...
  function subjectName(id: string): string {
    return pickText(subjects?.items.find((s) => s.id === id)?.name ?? {})
  }

  async function handleToggle(topic: AdminTopic) {
    setPendingId(topic.id)
    try {
      await updateTopicStatus(topic.id, !topic.isActive)
  ...
  async function handleArchiveToggle(topic: AdminTopic) {
    setPendingId(topic.id)
    try {
      if (topic.status === 'archived') await restoreTopic(topic.id)
      else await archiveTopic(topic.id)
      await queryClient.invalidateQueries({ queryKey: ['admin', 'topics'] })
    } finally {
      setPendingId(null)
    }
        ...
        <CardContent className="p-0 sm:p-0">
          {isError ? (
            <div className="p-4 sm:p-5">
              <ErrorState title="Couldn't load topics" onRetry={() => void refetch()} />
            </div>
          ) : isPending || !data ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-muted-foreground border-b text-left text-xs">
                    ...
                    <th className="px-4 py-2.5 font-medium sm:px-5">Subject</th>
                ...
                <tbody>
                  {data.items.map((topic) => (
                    ...
                    >
                      <td className="px-4 py-2.5 font-medium sm:px-5">{topic.slug}</td>
                      <td className="px-4 py-2.5 sm:px-5">{pickText(topic.name)}</td>
                      <td className="px-4 py-2.5 sm:px-5">
                        {subjectName(topic.subjectId)}
                      </td>
                      <td className="px-4 py-2.5 sm:px-5">
                        <Badge
                          variant={STATUS_BADGE[topic.status]}
                      ...
                      <td className="px-4 py-2.5 sm:px-5">
                        <div className="flex flex-wrap gap-1.5">
                          ...
                          <Button
                            variant={
                              topic.status === 'archived' ? 'outline' : 'destructive'
                            }
                            size="sm"
                            loading={pendingId === topic.id}
                            onClick={() => void handleArchiveToggle(topic)}
                          >
...
function TopicForm({
  ...
  const [nameEn, setNameEn] = useState(topic?.name.en ?? '')
  const [nameTa, setNameTa] = useState(topic?.name.ta ?? '')
  ...
  return (
    <Card>
      <CardContent className="flex flex-col gap-3 p-4 sm:p-5">
        <h3 className="text-sm font-semibold">{topic ? 'Edit Topic' : 'New Topic'}</h3>
        {error && (
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button
            ...
            disabled={!nameEn.trim() || !slug.trim() || !subjectId}
